  ...
  <v-sheet class="ma-2 pa-2">
    <v-row>
      ...
      >
        ...
        <v-sheet class=" ma-2 pa-2 ">
          <v-card
            @drop="onDrop($event, categori.name)" @dragover.prevent @dragenter.prevent class=" mt-16"
            color="primary"
            :subtitle="'Tổng số thẻ: '+desserts.filter(items=>items.progress=== categori.name ).length "
            :title="categori.name" variant="elevated">
            <div class="task ma-4 pa-4 rounded">
              <div class="scrollable">
                <div v-for="(item ) in desserts.filter(items=>items.progress=== categori.name )"
                     :key="item.taskName"
                     draggable="true"
              ...
              <div class="v-picker--full-width  ma-2 rounded text-black">
                <v-text-field label="Add task" v-model="categori.input" variant="outlined"></v-text-field>
                <v-btn color="success" @click="addTodo(categori)" variant="flat">
                  Add
                </v-btn>
              </div>
            </div>
...
export default {
  ...
  methods: {
    ...
    addTodo(categories) {
      if (categories.input !== "") {
        const newTask = {
          taskName: categories.input,
          progress: categories.name,
        }
